...
function displaySettings(event, id, flag) {
  var rnumber = 0;
  var container;
  if(id) {
    rnumber = id
...
document.getElementById('canvas').addEventListener('mousedown', function canvasClick(e) {
  ...
  x = Math.round((x / canvas.width) * pozyx_x_max);
  y = Math.round((y / canvas.height) * pozyx_y_max);
  var shiftFlag = false;
  var onRobot = false;
  current_robot_list.forEach((robot) => {
    if(Math.abs(robot.x - x) < 80 && Math.abs(robot.y - y) < 80) {
